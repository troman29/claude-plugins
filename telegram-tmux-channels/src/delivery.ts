...
export async function watchDelivery(
  ...
  if (ack === 'ok') {
    // Видно в логе, иначе быстрый путь неотличим от медленного и его нечем проверить
    d.log(`delivery: ${key} — стаб подтвердил доставку`)
    return 'landed' // гадать по транскрипту незачем
  }
