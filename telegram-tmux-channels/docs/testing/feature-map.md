...
| 2 | `trusted-groups` | авто-бинд новых топиков, режимы folder/worktree, хук `wt.py`, транслит кириллицы, выбор режима, **база ворктри** (`worktree.base`, кнопка на базу) и **суффикс `-2`** при занятом имени ветки | `pendingTopics`/`pendingModeChoice`, `runAutoTopic` | 🟡 (TG1 авто-бинд ✅ e2e, TG3 exclude ✅; юниты translit/exclude/merge ✅; TODO worktree+хук, TG4) |
| 3 | `session-lifecycle` | `/resume` `/new` `/restart` `/close`, автоспавн хаба, авто-ack стартовых промптов, не-двойной-старт, `reviveBoundSessions` | `spawnSession`, `restartSession`, `stopSession` | 🟡 (SL4 restart+контекст ✅, SL5 не-двойной ✅, SL6 restart мёртвой Codex-сессии ✅; SL1-3 ещё требуют отдельных E2E) |
...
| 9 | `ops-commands` | `/compact` `/clear` `/esc`(дренаж очереди) `/enter` `/status` `/model` `/queue`(придержать до конца хода) | ops-диспатч в `hub.ts`, `parseOpsCommand` | 🟡 (OC3 esc-прерывание ✅, OC4 compact ✅, OC1/OC2/OC7 ✅; TODO clear/enter) |
...
| 13 | `reply-fallback` | добор ответа из транскрипта на turnend, если агент не отправил; молчит, если агент в этом ходе уже отвечал (иначе дублировал дописанное посреди хода) | `forwardFallbackReply`, `session-id.ts` | 🟢 (RF1 авто-досыл ✅ e2e, RF2/RF3/RF4 ✅) |
...
| 16 | `pane-detectors` | детект компакции/воркфлоу/ошибок в `pollScreens` | `handleCompaction`/`handleWorkflow`/`handleErrors` | 🟡 (PD1 компакция прогресс→готово ✅ e2e, PD5 один пост на два источника ✅ e2e, PD4 анти-ложняк ✅ юнит; PD2 ошибки / PD3 workflow — e2e ⬜, парсеры юнитами ✅) |
